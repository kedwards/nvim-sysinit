...
-- User commands for managing the LSP loader system

local M = {}

--- Setup user commands for LSP management
function M.setup()
  ...
  -- Show tool name mappings
  vim.api.nvim_create_user_command("LspShowToolMappings", function()
    print("=== Tool Name Mappings (Config -> Mason) ===")
    local loader = require("lsp.loader")
    local tools = loader.get_tools()
    ...
    for tool_type, tool_list in pairs(tools) do
      if #tool_list > 0 then
        print(string.format("\n%s:", tool_type:upper()))
        for _, tool in ipairs(tool_list) do
          -- Access the mason name mapping through the loader's get_mason_name function
          -- We'll need to expose this or recreate the logic here
          print(string.format("  %s -> (check Mason registry)", tool))
        end
      end
    end
    
    print("\nNote: Use :Mason to see available packages in the registry")
  end, {
    desc = "Show tool name mappings from config to Mason packages",
  })
  ...
  vim.api.nvim_create_user_command("LspShowFiletypeConfig", function(opts)
    ...
    complete = function()
      ...
      return {
        ...
        "html", "css", "json", "yaml", "markdown", "sh", "bash"
      }
    end,
  })
  ...
  vim.api.nvim_create_user_command("LspNewConfig", function(opts)
    ...
    local template = string.format([[-- %s Language Configuration
...
return {
  ...
  dap = {},
}]], name:gsub("^%l", string.upper), name, name, name, name, name, name, name)
